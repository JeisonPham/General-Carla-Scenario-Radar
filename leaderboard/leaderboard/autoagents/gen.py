...
from leaderboard.autoagents import ply
import logging
import queue
import struct
import math
...
class HDL64E(Sensor):
    ...
    def __init__(self, vehicle, world, actor_list, folder_output, transform):
        ...
        self.frame_output = os.path.join(folder_output, "frames")
        os.makedirs(self.frame_output) if not os.path.exists(self.frame_output) else [os.remove(f) for f in glob.glob(
            self.frame_output + "/*") if os.path.isfile(f)]
    ...
    def save(self):
        while not self.queue.empty():
            ...
            if self.i_packet % self.packet_per_frame == 0:
                ...
                print(np.float32(pts_all).shape, np.float32(ts_all).shape, np.uint32(semantic_all).shape)
                np.save(ply_file_path, np.concatenate([np.float32(pts_all), np.float32(ts_all)[:, None], np.uint32(semantic_all)], axis=1))
            ...
            with open(os.path.join(self.calib_output, "full_poses_lidar.txt"), 'a') as posfile:
    ...
    def set_attributes(self, blueprint_library):
        ...
        lidar_bp.set_attribute('range', '100')  # 80.0 m
