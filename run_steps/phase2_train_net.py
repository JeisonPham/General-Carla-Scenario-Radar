...
class Engine(object):
    ...
    def __init__(self, device, log_dir, cur_epoch=0, cur_iter=0, logger=None):
    ...
    def train(self, model, dataloader_train, config, optimizer):
        ...
        for cur_num, data in enumerate(dataloader_train):
            ...
            # model input
            pred_wp = model(fronts, lidars, maps, None, radar, radar_adj, target_point, gt_velocity)

            gt_waypoints = [torch.stack(data['waypoints'][i], dim=1).to(self.device, dtype=torch.float32) for i in
                            range(config.seq_len, len(data['waypoints']))]
            ...
            if afc.local_rank == 0:
                ...
                self.logger.log_metric("loss", loss.item(), step=self.cur_iter, epoch=self.cur_epoch)
        ...
        with torch.no_grad():
            ...
            if num_batches != 0:
                ...
                if afc.local_rank == 0:
                    ...
                    self.logger.log_metric("val loss", wp_loss, step=self.cur_epoch)
...
def main(args: DictConfig):
    ...
    if n_gpu > 2 and args.is_multi_gpu:
        os.environ['RANK'] = str(afc.local_rank)
    ...
    config.train_data = os.path.join(args.absolute_path, args.data_folder.split('/')[0],
                                     'pro_train_f' + args.data_folder.split('/')[1])
    config.val_data = os.path.join(args.absolute_path, args.data_folder.split('/')[0],
                                   'pro_eval_f' + args.data_folder.split('/')[1])
    ...
    if afc.local_rank == 0:
        print(
            f"{bc.OKGREEN} Data Path:  {str(config.train_data)} {bc.ENDC}, {bc.OKCYAN}total train frame: {len(train_set)} {bc.ENDC}")
    ...
    experiment = Experiment(**OmegaConf.to_container(args.comet), disabled=True)
    experiment.log_parameters(args)

    optimizer = optim.AdamW(model.parameters(), lr=args.lr)
    trainer = Engine(afc.device, args.logdir, logger=experiment)
    ...
    if args.is_multi_gpu and n_gpu > 1 and afc.local_rank == 0:
        ...
        dataloader_train = DataLoader(train_set, batch_size=args.batch_size, sampler=train_sampler, num_workers=8,
                                      pin_memory=True, collate_fn=collate_single_cpu)
        dataloader_val = DataLoader(val_set, batch_size=args.batch_size, sampler=val_sampler, num_workers=4,
                                    pin_memory=True, collate_fn=collate_single_cpu)
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[afc.local_rank],
                                                          output_device=afc.local_rank, find_unused_parameters=True)
    else:
    ...
    for epoch in range(trainer.cur_epoch, args.epochs):
        ...
        if afc.local_rank == 0:
            ...
            experiment.log_metric("time spent", ((time.time() - epoch_t_start)))
...
if __name__ == '__main__':
    ...
    if afc.local_rank == 0:
        print(f"{bc.OKGREEN}TOTAL RUNNING TIME{bc.ENDC}: --- %s hours --- Clean Memory %s" % (
        round((time.time() - start_time) / 3600, 2), gc.collect()))
        wandb.finish()
